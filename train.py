        ...
        for config in validation_configs:
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx, viewpoint in enumerate(config["cameras"]):
                    ...
                    ssim_list.append(
                        ssim_func(image[None], gt_image[None], data_range=1.0).mean()
                    )
                    lpips_list.append(lpips(image[None], gt_image[None]).mean())
                    ms_ssim_list.append(
                        ms_ssim(image[None], gt_image[None], data_range=1.0).mean()
                    )
                    alex_lpips_list.append(
                        alex_lpips(image[None], gt_image[None]).mean()
                    )
                ...
                if progress_bar is None:
                    print(
                        "\n[ITER {}] Evaluating {}: L1 {} PSNR {} SSIM {} LPIPS {} MS SSIM{} ALEX_LPIPS {}".format(
                            iteration,
                            config["name"],
                            l1_test,
                            psnr_test,
                ...
                if tb_writer:
                    ...
                    tb_writer.add_scalar(
                        config["name"] + "/loss_viewpoint - ssim", test_ssim, iteration
                    )
                    tb_writer.add_scalar(
                    ...
                    tb_writer.add_scalar(
                        config["name"] + "/loss_viewpoint - ms-ssim",
                        test_ms_ssim,
                        iteration,
                    )
                    tb_writer.add_scalar(
                        config["name"] + "/loss_viewpoint - alex-lpips",
                        test_alex_lpips,
                        iteration,
                    )
