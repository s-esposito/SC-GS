...
def mask_images(img_path, msk_path, sv_path=None, no_mask=False):
    ...
    image_names = [
        img for img in image_names if img.endswith(".png") or img.endswith(".jpg")
    ]
    msk_names = sorted(os.listdir(msk_path))
    msk_names = [
        img for img in msk_names if img.endswith(".png") or img.endswith(".jpg")
    ]
    ...
    if not os.path.exists(sv_path) and not os.path.exists(
        sv_path + "../unmasked_images/"
    ):
        os.makedirs(sv_path)
    else:
        return sv_path
