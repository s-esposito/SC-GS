...
class App(QWidget):
    ...
    def save(self):
        folder_path = (
            str(QFileDialog.getExistingDirectory(self, "Select Save Directory"))
    ...
    def enter_local_control(self):
        ...
        init_interaction = CropperInteraction(
            self.processor.images[:, self.cursur],
            prev_soft_mask,
            self.processor.pad,
            self.local_bb,
        )
...
def seg_video(
    img_path=None, video_path=None, MIVOS_PATH="/home/yihua/nips2022/code/repos/MiVOS/"
):

    # Arguments parsing
    parser = ArgumentParser()
    parser.add_argument(
        "--prop_model", default=MIVOS_PATH + "saves/propagation_model.pth"
    )
