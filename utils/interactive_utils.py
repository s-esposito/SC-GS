...
class DeformKeypoints:
    ...
    def get_deformed_kpt_np(self, rate=1.0):
        return (
            np.array(self.keypoints3d_list)
            + np.array(self.keypoints3d_delta_list) * rate
        )
    ...
    def add_kpts(self, keypoints_coord, keypoints_idx, expand=False):
        ...
        selective_keypoints_idx_list = (
            [] if not expand else self.selective_keypoints_idx_list
        )
        for idx in range(len(keypoints_idx)):
            if not self.contain_kpt(keypoints_idx[idx].item()):
                ...
                self.keypoints3d_delta_list.append(
                    np.zeros_like(self.keypoints3d_list[-1])
                )
    ...
    def get_rotation_center(
        ...
        selected_rotation_points = self.get_deformed_kpt_np()[
            self.selective_rotation_keypoints_idx_list
        ]
        return selected_rotation_points.mean(axis=0)

    def get_selective_center(
        self,
    ):
    ...
    def set_rotation_delta(self, rot_mat):
        ...
        delta = (
            kpts3d - np.array(self.keypoints3d_list)[self.selective_keypoints_idx_list]
        )
        for id, idx in enumerate(self.selective_keypoints_idx_list):
            self.keypoints3d_delta_list[idx] = delta[id]
