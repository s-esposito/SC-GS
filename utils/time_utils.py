        ...
        if global_step > self.cur_step.item():
            if self.n_masking_step <= 0 or global_step is None or not self.training:
                self.mask = torch.ones(
                    self.N_freqs, dtype=torch.float32, device=torch.device("cuda:0")
                )
            else:
                self.mask = (
                    1.0
                    - torch.cos(
                        ...
                        * (
                            ...
                            - torch.arange(
                                0, self.N_freqs, device=torch.device("cuda:0")
                            )
                        ).clamp(0, 1)
                    )
                ) / 2.0
...
class StaticNetwork(nn.Module):
    ...
    def forward(self, x, t, **kwargs):
        if self.return_tensors:
            return_dict = {
                "d_xyz": torch.zeros_like(x),
                "d_rotation": torch.zeros_like(x[..., [0, 0, 0, 0]]),
                "d_scaling": torch.zeros_like(x),
                "local_rotation": torch.zeros_like(x[..., [0, 0, 0, 0]]),
                "hidden": None,
                "d_opacity": None,
                "d_color": None,
            }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.progressive_brand_time:
            self.embed_time_fn = ProgressiveBandFrequency(
                in_channels=1, n_frequencies=self.t_multires
            )
        ...
        if self.pred_color:
            if self.resnet_color:
                in_dim = (
                    ...
                    else self.linear[0].weight.shape[-1] + W
                )  # Color depends on Direction or Position-Time
                # self.gaussian_color = MLP(dim_in=in_dim, dim_out=3, n_neurons=W, n_hidden_layers=3, skip=[])
                self.gaussian_color = nn.Sequential(
                    nn.Linear(in_dim, W),
                    nn.ReLU(),
                    nn.Linear(W, W),
                    nn.ReLU(),
                ...
                self.color_hash_encoding = ProgressiveBandHashGrid(
                    in_channels=4, start_level=16, n_levels=16
                )
                self.gaussian_color = MLP(
                    dim_in=self.color_hash_encoding.n_output_dims + W,
                    dim_out=3,
                    n_neurons=W // 4,
                    n_hidden_layers=3,
                )
        ...
        if self.pred_color:
            if self.resnet_color:
                if self.color_wrt_dir:
                    ...
                    return_dict["d_color"] = self.gaussian_color(
                        torch.cat([x_emb, t_emb, h], dim=-1)
                    )
            elif self.hash_color:
                return_dict["d_color"] = self.gaussian_color(
                    torch.cat(
                        [self.color_hash_encoding(torch.cat([x, t], dim=-1)), h], dim=-1
                    )
                )
            else:
...
class ProgressiveBandHashGridCosine(nn.Module):
    def __init__(
        ...
        self.mask = torch.ones(
            self.n_output_dims, dtype=torch.float32, device=torch.device("cuda:0")
        )
        ...
        if self.n_masking_step <= 0 or global_step is None or global_step < 0:
            ...
            self.mask[start_idx:] = (
                ...
                - torch.cos(
                    ...
                    * (
                        ratio * band_len
                        - torch.arange(0, band_len, device=torch.device("cuda:0"))
                    ).clamp(0, 1)
                )
            ) / 2.0
            # print(f"Update mask of Freq: {global_step}/{self.n_masking_step} {self.mask}")
...
class MLP(nn.Module):
    def __init__(
        ...
        for i in range(self.n_hidden_layers - 1):
            ...
            hidden_in_dim = (
                self.n_neurons + dim_in if i in self.skip else self.n_neurons
            )
...
class HashDeformNetwork(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "bbox",
            (
                torch.tensor([-2, 2]).float().cuda()
                if bbox is None
                else torch.tensor(bbox).float().cuda()
            ),
        )
        self.hashgrid = ProgressiveBandHashGridCosine(
            in_channels=4 if hash_time else 3,
            start_level=6,
            n_levels=12,
            start_step=1000,
            update_steps=1000,
        ...
        self.mlp = mlp_type(
            dim_in=(
                self.hashgrid.n_output_dims
                if hash_time
                else self.hashgrid.n_output_dims + self.time_embed_dim
            ),
            dim_out=hidden_dim,
            n_neurons=hidden_dim,
            n_hidden_layers=num_layers if hash_time else num_layers + 2,
            out_act=nn.Identity(),
            skip=[2] if not hash_time else [],
        )
        self.translate_layer = MLP(
            dim_in=hidden_dim, dim_out=3, n_neurons=64, n_hidden_layers=1
        )
        self.rotation_layer = MLP(
            dim_in=hidden_dim, dim_out=4, n_neurons=64, n_hidden_layers=1
        )
        self.scaling_layer = MLP(
            dim_in=hidden_dim, dim_out=3, n_neurons=64, n_hidden_layers=1
        )
        self.log_scale_range = np.log(scale_range)
        if not self.hash_time:
            nn.init.normal_(self.mlp.layers[-1].weight, mean=0.0, std=1e-5)
        self.local_frame = local_frame
        if self.local_frame:
            self.local_rotation_layer = MLP(
                dim_in=hidden_dim, dim_out=4, n_neurons=64, n_hidden_layers=1
            )
        self.pred_opacity = pred_opacity
        if self.pred_opacity:
            self.opacity_layer = MLP(
                dim_in=hidden_dim, dim_out=1, n_neurons=64, n_hidden_layers=1
            )
        self.pred_color = pred_color
        if self.pred_color:
            self.color_layer = MLP(
                dim_in=hidden_dim, dim_out=3, n_neurons=64, n_hidden_layers=1
            )
    ...
    def forward(self, x, t, **kwargs):
        ...
        if self.hash_time:
            ...
            t_embed = (
                t_embed / self.time_embed_dim * self.hashgrid.n_output_dims
            )  # Align the scale
            embed = torch.cat([x_embed, t_embed], dim=-1)
        hidden = self.mlp(embed).float()
        translate = self.translate_layer(hidden)
        rotation = self.rotation_layer(hidden) + torch.tensor(
            [1.0, 0, 0, 0]
        ).float().to(hidden.device)
...
class ControlNodeWarp(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.hyper_dim = (
            hyper_dim if not skinning else 0
        )  # skinning should not be with hyper
        self.is_blender = is_blender
        self.pred_opacity = pred_opacity
        self.pred_color = pred_color
        self.max_d_scale = max_d_scale
        ...
        if self.is_scene_static:
            ...
            self.network = HashDeformNetwork(
            ...
            self.network = DeformNetwork(
                is_blender=is_blender,
                local_frame=local_frame,
                pred_opacity=pred_opacity,
                pred_color=pred_color,
                progressive_brand_time=progressive_brand_time,
                max_d_scale=max_d_scale,
            ).cuda()
        ...
        if not self.skinning:
            ...
            if self.with_node_weight:
                self._node_weight = nn.Parameter(
                    torch.zeros_like(self.nodes[:, :1]), requires_grad=with_node_weight
                )
    ...
    def trainable_parameters(self):
        if self.skinning:
            return [
                {"params": list(self.network.parameters()), "name": "deform"},
            ...
            return [
                {"params": list(self.network.parameters()), "name": "deform"},
                {
                    "params": [self.nodes, self._node_radius, self._node_weight],
                    "name": "nodes",
                },
            ]
        else:
    ...
    def init(
        ...
        **kwargs,
        ...
        if self.use_hash and reset_bbox:
            self.network.bbox.data = (
                torch.tensor([init_pcl.min() - 0.1, init_pcl.max() + 0.1])
                .float()
                .cuda()
            )
        ...
        if keep_all or self.node_num > init_pcl.shape[0]:
            self.nodes = nn.Parameter(
                torch.cat(
                    [
                        init_pcl.float(),
                        1e-2
                        * torch.ones([init_pcl.shape[0], self.hyper_dim])
                        .float()
                        .cuda(),
                    ],
                    dim=-1,
            ...
            init_nodes_idx = farthest_point_sample(
                pcl_to_samp.detach()[None], self.node_num
            )[0]
            self.nodes.data = nn.Parameter(
                torch.cat(
                    [
                        init_pcl[init_nodes_idx].float(),
                        1e-2
                        * torch.ones([self.node_num, self.hyper_dim]).float().cuda(),
                    ],
                    dim=-1,
                )
            )
        ...
        if self.skinning:
            if "feature" in kwargs:
                ...
                radius = 0.1 * scene_range + 1e-7
                initial_weights = -torch.log(
                    (init_pcl[:, None] - self.nodes[None, ..., :3]).square().sum(dim=-1)
                    / radius**2
                )
            ...
            if keep_all or self.node_num > init_pcl.shape[0]:
                self._node_radius = nn.Parameter(
                    torch.log(0.1 * scene_range + 1e-7)
                    * torch.ones([self.node_num]).float().to(scene_range.device)
                )
                self._node_weight = nn.Parameter(
                    torch.zeros_like(torch.zeros_like(self.nodes[:, :1]))
                )
            else:
                ...
                self._node_weight.data = torch.zeros_like(
                    torch.zeros_like(self.nodes[:, :1])
                )
        self.gs = None
        if force_gs_keep_all:
            self.init_gaussians(
                init_pcl=init_pcl, with_motion_mask=as_gs_force_with_motion_mask
            )
        else:
            self.init_gaussians(
                init_pcl=self.nodes[..., :3],
                with_motion_mask=as_gs_force_with_motion_mask,
            )
        self.as_gaussians.training_setup(opt)
        print(
            f"Control node initialized with {self.nodes.shape[0]} from {init_pcl.shape[0]} points."
        )
        ...
        if self.skinning:
            ...
            if self.cached_nn_weight and self.nn_weight is not None:
                ...
                if self.hyper_dim > 0 and feature is not None:
                    x = torch.cat(
                        [x.detach(), feature[..., : self.hyper_dim]], dim=-1
                    )  # cat with hyper coor
                K = self.K if K is None else K
                # Weights of control nodes
                nodes = (
                    self.nodes[..., :3].detach() if nodes is None else nodes[..., :3]
                )
                if feature is not None:
                    nodes = torch.cat(
                        [nodes[..., :3].detach(), self.nodes[..., 3:]], dim=-1
                    )  # Freeze the first 3 coordinates for deformation mlp input
                nn_dist, nn_idxs, _ = pytorch3d.ops.knn_points(
                    x[None], nodes[None], None, None, K=K
                )  # N, K
    ...
    def cal_nn_weight_floyd(
        ...
        if not hasattr(self, f"{cache_name}_nn_dist") or (
            t0 is not None
            and (getattr(self, f"{cache_name}_t") - t0).abs().max() > 1e-2
        ):
        ...
        nn_dist, nn_idxs, _ = pytorch3d.ops.knn_points(
            x[None], cur_node[None], None, None, K=1
        )  # N, K
        nn_dist, nn_idxs = nn_dist[0, :, 0], nn_idxs[0, :, 0]  # N
        knn_dist, knn_idxs = (
            getattr(self, f"{cache_name}_nn_dist")[nn_idxs] + nn_dist[:, None],
    ...
    def node_deform(self, t, detach_node=True, **kwargs):
        ...
        values = {
            key: (
                values[key].view(*tshape[:-1], values[key].shape[-1])
    ...
    def sample_node_deform(self, samp_num=512, sv_path="./deform"):
        ...
        while start < samp_num:
            ...
            start = end
        values_np = {
            key: values[key].detach().cpu().numpy() for key in values if key != "hidden"
        }
        np.savez(sv_path, **values_np, nodes=self.nodes.detach().cpu().numpy())
        print(
            f"Successfully save {values_np.keys()} into {sv_path}! Without hidden features!"
        )
    ...
    def get_trajectory(self, t_samp_num=8):
        ...
        for key in node_deform:
            node_deform[key] = (
                node_deform[key][:, 0] if node_deform[key] is not None else None
            )
    ...
    def arap_loss_with_rot(self, t_samp_num=8):
        ...
        arap_error, rot_error = arap_deformation_loss(
            trajectory=trajectory,
            node_radius=self.node_radius.detach(),
            trajectory_rot=trajectory_rot,
            with_rot=not self.d_rot_as_res,
        )
    ...
    def p2dR(self, p, p0=None, K=8, as_quat=True, mode="trajectory", t0=None):
        ...
        if mode == "trajectory":
            ...
            nn_dist, nn_idx, _ = pytorch3d.ops.knn_points(
                trajectory[None], trajectory[None], None, None, K=K + 1, return_nn=False
            )
            nn_dist, nn_idx = nn_dist[0, :, 1:], nn_idx[0, :, 1:]
            nn_weight = torch.softmax(nn_dist / nn_dist.mean(), dim=-1)
            edges = (
                torch.gather(
                    t0_nodes[:, None].expand([nodes.shape[0], K, nodes.shape[-1]]),
                    dim=0,
                    index=nn_idx[..., None].expand(
                        [nodes.shape[0], K, nodes.shape[-1]]
                    ),
                )
                - t0_nodes[:, None]
            )
        elif mode == "floyd":
            nn_weight, _, nn_idx = self.cal_nn_weight_floyd(
                x=p,
                t0=t0,
                cur_node=p0,
                K=K + 1,
            ...
            edges = (
                torch.gather(
                    p0[:, None].expand([nodes.shape[0], K, nodes.shape[-1]]),
                    dim=0,
                    index=nn_idx[..., None].expand(
                        [nodes.shape[0], K, nodes.shape[-1]]
                    ),
            ...
            nn_dist, nn_idx, nn_nodes = pytorch3d.ops.knn_points(
                nodes[None], nodes[None], None, None, K=K + 1, return_nn=True
            )
            nn_dist, nn_idx, nn_nodes = (
                nn_dist[0, :, 1:],
                nn_idx[0, :, 1:],
                nn_nodes[0, :, 1:],
            )
        ...
        edges_t = (
            torch.gather(
                p[:, None].expand([p.shape[0], K, p.shape[-1]]),
                dim=0,
                index=nn_idx[..., None].expand([p.shape[0], K, p.shape[-1]]),
            )
            - p[:, None]
        )
        ...
        W = torch.zeros(
            [edges.shape[0], K, K], dtype=torch.float32, device=edges.device
        )
    ...
    def arap_loss(self, t=None, delta_t=0.05, t_samp_num=2):
        t = (
            torch.rand([]).cuda()
            if t is None
            else t.squeeze() + delta_t * (torch.rand([]).cuda() - 0.5)
        )
        t_samp = torch.rand(t_samp_num).cuda() * delta_t + t - 0.5 * delta_t
        t_samp = t_samp[None, :, None].expand(self.node_num, t_samp_num, 1)  # M, T, 1
    ...
    def elastic_loss(self, t=None, delta_t=0.005, K=2, t_samp_num=8):
        ...
        nn_weight, _, nn_idx = self.cal_nn_weight(
            x=self.nodes[..., :3].detach(), feature=self.nodes[..., 3:], K=K + 1
        )
    ...
    def acc_loss(self, t=None, delta_t=0.005):
        # Calculate nodes translate
        t = (
            torch.rand([]).cuda()
            if t is None
    ...
    def geodesic_distance_floyd(self, cur_node, K=8):
        ...
        for i in range(nn_dist.shape[0]):
            dist_mat = torch.minimum(
                (dist_mat[:, i, None] + dist_mat[None, i, :]), dist_mat
            )
        return dist_mat
    ...
    def forward(
        ...
        node_scaling_bias=None,
        animation_d_values=None,
        **kwargs,
    ):
        if t.dim() == 0:
            t = self.expand_time(t)
        x = x.detach()
        rot_bias = torch.tensor([1.0, 0, 0, 0]).float().to(x.device)
        ...
        node_trans, node_rot, node_scale = (
            node_attrs["d_xyz"],
            node_attrs["d_rotation"],
            node_attrs["d_scaling"],
        )
        ...
        if self.local_frame:
            ...
            Ax = (
                torch.einsum(
                    "nkab,nkb->nka", local_rot_matrix[nn_idx], x[:, None] - nn_nodes
                )
                + nn_nodes
                + node_trans[nn_idx]
            )
        ...
        if not self.d_rot_as_res:
            ...
            rotation = (
                (node_rot[nn_idx] * nn_weight[..., None]).sum(dim=1) - rot_bias
            ) * motion_mask + rot_bias
            if node_trans_bias is not None:
                # Calculate nodes and gs at t0
                with torch.no_grad():
                    cur_node = self.nodes[..., :3] + node_trans
                    cur_gs = x + translate
                    cur_nn_weight, _, cur_nn_idx = self.cal_nn_weight(
                        x=cur_gs, feature=None, nodes=cur_node, K=32
                    )

                    node_rot_R = quaternion_to_matrix(node_rot)[cur_nn_idx]
                    gs_init = x + translate
                    # Aligh the relative distance considering the rotation
                    gs_t = nodes_t[cur_nn_idx] + torch.einsum(
                        "gkab,gkb->gka",
                        node_rot_R,
                        (gs_init[:, None] - nodes_t_init[cur_nn_idx]),
                    )
            ...
            if node_trans_bias is not None:
                with torch.no_grad():
                    ...
                    cur_nn_weight, _, cur_nn_idx = self.cal_nn_weight_floyd(
                        x=cur_gs,
                        t0=t,
                        cur_node=cur_node,
                        K=8,
                    ...
                    gs_t = nodes_t[cur_nn_idx] + torch.einsum(
                        "gkab,gkb->gka",
                        d_nn_node_rot_R,
                        (gs_init[:, None] - cur_node[cur_nn_idx]),
                    )
                    gs_t_avg = (gs_t * cur_nn_weight[..., None]).sum(dim=1)
                    translate = gs_t_avg - x
                    return_dict["d_xyz"] = translate * motion_mask
                    return_dict["d_rotation_bias"] = (
                        (node_rot_bias[cur_nn_idx] * cur_nn_weight[..., None]).sum(
                            dim=1
                        )
                        - rot_bias
                    ) * motion_mask + rot_bias
        ...
        if self.pred_opacity:
            ...
            d_opacity = (node_opacity[nn_idx] * nn_weight[..., None]).sum(
                dim=1
            ) * motion_mask
        ...
        if self.pred_color:
            ...
            d_color = (node_color[nn_idx] * nn_weight[..., None]).sum(
                dim=1
            ) * motion_mask
    ...
    def as_gaussians(self):
        if not hasattr(self, "gs") or self.gs is None:
            ...
            self.gs = StandardGaussianModel(
                sh_degree=0, all_the_same=True, with_motion_mask=False
            )  # blender datas are all dynamic
            self.gs.create_from_pcd(pcd=pcd, spatial_lr_scale=0.0, print_info=False)
    ...
    def init_gaussians(self, init_pcl, with_motion_mask):
        if not hasattr(self, "gs") or self.gs is None:
            ...
            self.gs = StandardGaussianModel(
                sh_degree=0, all_the_same=True, with_motion_mask=with_motion_mask
            )  # blender datas are all dynamic
            self.gs.create_from_pcd(pcd=pcd, spatial_lr_scale=0.0, print_info=False)
        return self.gs
    ...
    def as_gaussians_visualization(self):
        ...
        pcd = BasicPointCloud(
            points=self.nodes[..., :3].detach(),
            colors=self.nodes_color_visualization,
            normals=self.nodes[..., :3].detach(),
        )
        gs = GaussianModel(sh_degree=0, with_motion_mask=False)
        gs.create_from_pcd(pcd=pcd, spatial_lr_scale=0.0, print_info=False)
    ...
    def cal_node_importance(self, x: torch.Tensor, K=None, weights=None, feature=None):
        ...
        node_importance.index_add_(
            dim=0,
            index=nn_idxs.view(-1),
            source=(nn_weight * weights[:, None]).view(-1),
        )
        node_edge_count.index_add_(
            dim=0, index=nn_idxs.view(-1), source=nn_weight.view(-1)
        )
        avg_affected_x.index_add_(
            dim=0,
            index=nn_idxs.view(-1),
            source=(
                (nn_weight * weights[:, None]).view(-1, 1)
                * x[:, None]
                .expand(*nn_weight.shape, x.shape[-1])
                .reshape(-1, x.shape[-1])
            ),
        )
    ...
    def densify(
        ...
        if selected_pts_mask.sum() > 0 or pruned_pts_mask.sum() > 0:
            print(
                f"Add {selected_pts_mask.sum()} nodes and prune {pruned_pts_mask.sum()} nodes. ",
                end="",
            )
        else:
            return
        ...
        if selected_pts_mask.sum() > 0:
                ...
                for i in param_idx:
                    ...
                    if stored_state is not None:
                        ...
                        group["params"][i] = nn.Parameter(
                            torch.cat(
                                (group["params"][i], extension_tensor), dim=0
                            ).requires_grad_(True)
                        )
            ...
            self.nodes_color_visualization = torch.cat(
                [self.nodes_color_visualization, torch.ones_like(new_nodes[..., :3])],
                dim=0,
            )
        ...
        if pruned_pts_mask.shape[0] < self.nodes.shape[0]:
            pruned_pts_mask = torch.cat(
                [
                    pruned_pts_mask,
                    torch.zeros([self.nodes.shape[0] - pruned_pts_mask.shape[0]])
                    .to(pruned_pts_mask.device)
                    .to(pruned_pts_mask.dtype),
                ]
            )
        if pruned_pts_mask.sum() > 0:
            pruned_pts_mask = ~pruned_pts_mask
            self.nodes_color_visualization = self.nodes_color_visualization[
                pruned_pts_mask
            ]
            optimizable_tensors = {}
            param_list = self.param_names
            ...
            for group in optimizer.param_groups:
                ...
                for i in param_idx:
                    ...
                    if stored_state is not None:
                        ...
                        del optimizer.state[group["params"][i]]
                        group["params"][i] = nn.Parameter(
                            (group["params"][i][pruned_pts_mask].requires_grad_(True))
                        )
                        optimizer.state[group["params"][i]] = stored_state
        ...
        self.gs.densify_and_split(
            selected_pts_mask=selected_pts_mask, N=1, without_prune=True
        )
