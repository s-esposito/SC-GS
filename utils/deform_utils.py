...
def produce_edge_matrix_nfmt(
    verts: torch.Tensor, edge_shape, ii, jj, nn, device="cuda"
) -> torch.Tensor:
    ...
    if trajectory is None:
        if mode == "floyd":
            ...
            knn_res = pytorch3d.ops.knn_points(
                points[None], points[None], None, None, K=K + 1
            )
            # Remove themselves
            nn_dist, nn_idx = (
                knn_res.dists[0, :, 1:],
        ...
        trajectory = (
            trajectory.reshape([Nv, -1]) / trajectory.shape[1]
...
def estimate_rotation(source, target, ii, jj, nn, K=10, weight=None, sample_idx=None):
    ...
    source_edge_mat = produce_edge_matrix_nfmt(
        source, (Nv, K, 3), ii, jj, nn
    )  # [Nv, K, 3]
    target_edge_mat = produce_edge_matrix_nfmt(
        target, (Nv, K, 3), ii, jj, nn
    )  # [Nv, K, 3]
    if weight is None:
    ...
    S = torch.bmm(
        source_edge_mat.permute(0, 2, 1), torch.bmm(D, target_edge_mat)
    )  # [Nv, 3, 3]
...
def cal_arap_error(nodes_sequence, ii, jj, nn, K=10, weight=None, sample_num=512):
    ...
    if weight is None:
        ...
        weight[ii, nn] = 1
    source_edge_mat = produce_edge_matrix_nfmt(
        nodes_sequence[0], (Nv, K, 3), ii, jj, nn
    )  # [Nv, K, 3]
...
def arap_deformation_loss(
    trajectory, node_radius=None, trajectory_rot=None, K=50, with_rot=True
):
    init_pcl = trajectory[:, 0]
    radius = (
        torch.linalg.norm(init_pcl.max(dim=0).values - init_pcl.min(dim=0).values) / 8
    )
    ...
    with torch.no_grad():
        radius = (
            torch.linalg.norm(init_pcl.max(dim=0).values - init_pcl.min(dim=0).values)
            / 8
        )
    ...
    arap_error = (
        (weight[..., None] * (P_prime - torch.einsum("bxy,bky->bkx", R, P)))
        .square()
        .mean(dim=0)
        .sum()
    )
