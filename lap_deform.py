...
class LapDeform(nn.Module):
    def __init__(self, init_pcl, K=4, trajectory=None, node_radius=None):
        ...
        nn_dist, nn_idxs, _ = pytorch3d.ops.knn_points(
            init_pcl[None], init_pcl[None], None, None, K=K + 1
        )  # N, K
        nn_dist, nn_idxs = nn_dist[0, :, 1:], nn_idxs[0, :, 1:]
        nn_dist = 1 / (nn_dist + 1e-7)
        self.nn_idxs = nn_idxs
        self._weight = nn.Parameter(
            torch.log(nn_dist / (nn_dist.sum(dim=1, keepdim=True) + 1e-5) + 1e-5)
        )
        ...
        if self.mask_control_points:
            ...
            radius = (
                torch.linalg.norm(
                    self.init_pcl_reduced.max(dim=0).values
                    - self.init_pcl_reduced.min(dim=0).values
                )
                / 10
                * 3
            )
            ...
            self.arap_deformer = ARAPDeformer(
                self.init_pcl_reduced,
                radius=radius,
                K=30,
                point_mask=self.init_pcl_mask,
            ...
            radius = (
                torch.linalg.norm(
                    self.init_pcl.max(dim=0).values - self.init_pcl.min(dim=0).values
                )
                / 8
            )
            print("Set ball query radius to %f" % radius.item())
    ...
    def estimate_R(self, pcl, return_quaternion=True):
        old_edges = (
            torch.gather(
                input=self.init_pcl[:, None].repeat(1, self.K, 1),
                dim=0,
                index=self.nn_idxs[..., None].repeat(1, 1, 3),
            )
            - self.init_pcl[:, None]
        )  # N, K, 3
        edges = (
            torch.gather(
                input=pcl[:, None].repeat(1, self.K, 1),
                dim=0,
                index=self.nn_idxs[..., None].repeat(1, 1, 3),
            )
            - pcl[:, None]
    ...
    def energy_arap(self, pcl, prev_pcl):
        ...
        loss = cal_arap_error(
            node_seq,
            self.arap_deformer.ii,
            self.arap_deformer.jj,
            self.arap_deformer.nn,
            K=self.arap_deformer.K,
            weight=self.arap_deformer.normalized_weight,
        )
        return loss
    ...
    def deform(self, handle_idx, handle_pos, static_idx=None):
        ...
        return lstsq_with_handles(
            A=self.L, b=b, handle_idx=handle_idx, handle_pos=handle_pos
        )
    ...
    def deform_arap(self, handle_idx, handle_pos, init_verts=None, return_R=False):
        ...
        deformed_p, deformed_r, deformed_s = self.arap_deformer.deform(
            handle_idx, handle_pos, init_verts=init_verts, return_R=return_R
        )
        if self.mask_control_points:
            ...
            deformed_r_all = (
                ...
                .to(deformed_r.dtype)
                .to(deformed_r.device)
                .repeat(deformed_p_all.shape[0], 1)
            )
