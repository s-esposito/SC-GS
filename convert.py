...
parser.add_argument("--dataset_path", "-s", required=True, type=str)
...
if not args.skip_matching:
    ...
    os.system(colmap_command + " feature_extractor "\
        ...
        --image_path " + args.dataset_path + "/input \
...
files = os.listdir(args.dataset_path + "/sparse")
...
if(args.resize):
    ...
    files = os.listdir(args.dataset_path + "/images")
    # Copy each file from the source directory to the destination directory
    for file in files:
        source_file = os.path.join(args.dataset_path, "images", file)

        destination_file = os.path.join(args.dataset_path, "images_2", file)
        shutil.copy2(source_file, destination_file)
        os.system(magick_command + " mogrify -resize 50% " + destination_file)

        destination_file = os.path.join(args.dataset_path, "images_4", file)
        shutil.copy2(source_file, destination_file)
        os.system(magick_command + " mogrify -resize 25% " + destination_file)

        destination_file = os.path.join(args.dataset_path, "images_8", file)
        shutil.copy2(source_file, destination_file)
