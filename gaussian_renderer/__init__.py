...
def render_flow(
    ...
    **kwargs
    ...
    xyz_at_t1 = xyz_at_t2 = (
        carnonical_xyz.detach()
    )  # Detach coordinates of Gaussians here
    xyz_at_t1 = xyz_at_t1 + d_xyz1
    xyz_at_t2 = xyz_at_t2 + d_xyz2
    gaussians_homogeneous_coor_t2 = torch.cat(
        [xyz_at_t2, torch.ones_like(xyz_at_t2[..., :1])], dim=-1
    )
    full_proj_transform = (
        viewpoint_camera2.full_proj_transform
        if viewpoint_camera2 is not None
        else viewpoint_camera1.full_proj_transform
    )
    gaussians_uvz_coor_at_cam2 = gaussians_homogeneous_coor_t2 @ full_proj_transform
    gaussians_uvz_coor_at_cam2 = (
        gaussians_uvz_coor_at_cam2[..., :3] / gaussians_uvz_coor_at_cam2[..., -1:]
    )

    gaussians_homogeneous_coor_t1 = torch.cat(
        [xyz_at_t1, torch.ones_like(xyz_at_t1[..., :1])], dim=-1
    )
    gaussians_uvz_coor_at_cam1 = (
        gaussians_homogeneous_coor_t1 @ viewpoint_camera1.full_proj_transform
    )
    gaussians_uvz_coor_at_cam1 = (
        gaussians_uvz_coor_at_cam1[..., :3] / gaussians_uvz_coor_at_cam1[..., -1:]
    )
    ...
    if scale_const is not None:
        ...
        if d_rot_as_res:
            ...
            rotations = (
                pc.get_rotation
                if type(d_rotation1) is float
                else quaternion_multiply(d_rotation1, pc.get_rotation)
            )
        cov3D_precomp = None
    else:
        # If precomputed 3d covariance is provided, use it. If not, then it will be computed from
