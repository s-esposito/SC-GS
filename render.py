...
def render_set(
    ...
    render_path = os.path.join(run_path, name, "ours_{}".format(iteration), "renders")
    gts_path = os.path.join(run_path, name, "ours_{}".format(iteration), "gt")
    depth_path = os.path.join(run_path, name, "ours_{}".format(iteration), "depth")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        ssim_list.append(ssim_func(image[None], gt_image[None], data_range=1.0).mean())
        lpips_list.append(lpips(image[None], gt_image[None]).mean())
        ms_ssim_list.append(ms_ssim(image[None], gt_image[None], data_range=1.0).mean())
    ...
    # imageio.mimwrite(os.path.join(render_path, 'video.mp4'), renderings)
    ...
    print(
        "\n[ITER {}] Evaluating {}: PSNR {} SSIM {} LPIPS {} MS SSIM{} ALEX_LPIPS {}".format(
            iteration,
            name,
            psnr_test,
            ssim_test,
...
def interpolate_time(
    ...
    render_path = os.path.join(
        run_path, name, "interpolate_{}".format(iteration), "renders"
    )
    depth_path = os.path.join(
        run_path, name, "interpolate_{}".format(iteration), "depth"
    )
...
def interpolate_all(
    ...
    render_path = os.path.join(
        run_path, name, "interpolate_all_{}".format(iteration), "renders"
    )
    depth_path = os.path.join(
        run_path, name, "interpolate_all_{}".format(iteration), "depth"
    )
...
def render_sets(
    ...
    with torch.no_grad():
        ...
        deform.load_weights(dataset.run_path, iteration=iteration)
...
if __name__ == "__main__":
    ...
    render_sets(
        ...
        args.skip_test,
        args.mode,
        load2device_on_the_fly=args.load2gpu_on_the_fly,
    )
