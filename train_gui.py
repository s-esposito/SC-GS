...
class GUI:
    def __init__(
        self, args, dataset, opt, pipe, testing_iterations, saving_iterations
    ) -> None:
        ...
        if self.opt.progressive_train:
            self.opt.iterations_node_sampling = max(
                self.opt.iterations_node_sampling,
                int(
                    self.opt.progressive_stage_steps / self.opt.progressive_stage_ratio
                ),
            )
            self.opt.iterations_node_rendering = max(
                self.opt.iterations_node_rendering,
                self.opt.iterations_node_sampling + 2000,
            )
            print(
                f"Progressive trian is on. Adjusting the iterations node sampling to {self.opt.iterations_node_sampling} and iterations node rendering {self.opt.iterations_node_rendering}"
            )
        ...
        deform_loaded = self.deform.load_weights(dataset.run_path, iteration=-1)
        self.deform.train_setting(opt)

        gs_fea_dim = (
            self.deform.deform.node_num
            if args.skinning and self.deform.name == "node"
            else self.dataset.hyper_dim
        )
        ...
        if self.deform.name == "node" and not deform_loaded:
            if not self.dataset.is_blender:
                if self.opt.random_init_deform_gs:
                    ...
                    mean, scale = (
                        self.gaussians.get_xyz.mean(dim=0),
                        self.gaussians.get_xyz.std(dim=0).mean() * 3,
                    )
                    xyz = xyz * scale + mean
                    ...
                    self.deform.deform.init(
                        init_pcl=self.gaussians.get_xyz,
                        force_init=True,
                        opt=self.opt,
                        as_gs_force_with_motion_mask=self.dataset.as_gs_force_with_motion_mask,
                ...
                self.deform.deform.init(
                    ...
                    force_gs_keep_all=args.skinning,
                )
        ...
        self.iteration_node_rendering = (
            1 if self.scene.loaded_iter is None else self.opt.iterations_node_rendering
        )
        ...
        self.screenshot_sv_path = f"./screenshot/" + datetime.datetime.now().strftime(
            "%Y-%m-%d"
        )
    ...
    def animation_initialize(self, use_traj=True):
        ...
        if use_traj:
            ...
            trajectory = (
                self.deform.deform.node_deform(t=time_input)["d_xyz"]
                + gaussians.get_xyz[:, None]
            )
        else:
            trajectory = None
    ...
    def register_dpg(self):
            ...
            with dpg.collapsing_header(label="Initialize", default_open=True):
                ...
                with dpg.group(horizontal=True):
                    ...
                    def callback_save(sender, app_data, user_data):
                        ...
                        self.deform.save_weights(self.args.run_path, self.iteration)
                    ...
                    dpg.add_button(
                        label="screenshot",
                        tag="_button_screenshot",
                        callback=callback_screenshot,
                    )
                    ...
                    dpg.add_button(
                        label="render_traj",
                        tag="_button_render_traj",
                        callback=callback_render_traj,
                    )
                    dpg.bind_item_theme("_button_render_traj", theme_button)

                    def callback_render_traj(sender, app_data):
                        self.should_render_customized_trajectory_spiral = (
                            not self.should_render_customized_trajectory_spiral
                        )
                        if self.should_render_customized_trajectory_spiral:
                            dpg.configure_item(
                                "_button_render_traj_spiral", label="camera"
                            )
                        else:
                            dpg.configure_item(
                                "_button_render_traj_spiral", label="spiral"
                            )

                    dpg.add_button(
                        label="spiral",
                        tag="_button_render_traj_spiral",
                        callback=callback_render_traj,
                    )
                    dpg.bind_item_theme("_button_render_traj_spiral", theme_button)

                    def callback_cache_nn(sender, app_data):
                        self.deform.deform.cached_nn_weight = (
                            not self.deform.deform.cached_nn_weight
                        )
                        print(
                            f"Cached nn weight for higher rendering speed: {self.deform.deform.cached_nn_weight}"
                        )

                    dpg.add_button(
                        label="cache_nn",
                        tag="_button_cache_nn",
                        callback=callback_cache_nn,
                    )
            ...
            with dpg.collapsing_header(label="Train", default_open=True):
                ...
                with dpg.group(horizontal=True):
                    ...
                    def callback_save_deform_kpt(sender, app_data):
                        ...
                        save_obj(
                            path=self.args.run_path + "/deform_kpt.pickle",
                            obj=self.deform_keypoints,
                        )
                        print("Save kpt done!")

                    dpg.add_button(
                        label="save_deform_kpt",
                        tag="_button_save_deform_kpt",
                        callback=callback_save_deform_kpt,
                    )
                    ...
                    def callback_load_deform_kpt(sender, app_data):
                        ...
                        self.deform_keypoints = load_obj(
                            path=self.args.run_path + "/deform_kpt.pickle"
                        )
                        self.animation_time = self.deform_keypoints.t
                        with torch.no_grad():
                            animated_pcl, quat, ani_d_scaling = (
                                self.animate_tool.deform_arap(
                            ...
                            self.animation_trans_bias = (
                                animated_pcl - self.animate_tool.init_pcl
                            )
                    ...
                    dpg.add_button(
                        label="load_deform_kpt",
                        tag="_button_load_deform_kpt",
                        callback=callback_load_deform_kpt,
                    )
                ...
                with dpg.group(horizontal=True):
                    ...
                    def callback_animation_mode(sender, app_data):
                        with torch.no_grad():
                            ...
                            if self.is_animation:
                                if (
                                    not hasattr(self, "animate_tool")
                                    or self.animate_tool is None
                                ):
                                    self.animation_initialize()
                    ...
                    def callback_overlay(sender, app_data):
                        if self.showing_overlay:
                            ...
                            dpg.configure_item(
                                "_button_train_motion_gen", label="show overlay"
                            )
                        else:
                            self.showing_overlay = True
                            dpg.configure_item(
                                "_button_train_motion_gen", label="close overlay"
                            )
                    ...
                    def callback_save_ckpt(sender, app_data):
                        ...
                        deform_keypoint_files = sorted(
                            [
                                file
                                for file in os.listdir(
                                    os.path.join(self.args.run_path)
                                )
                                if file.startswith("deform_keypoints")
                                and file.endswith(".pickle")
                            ]
                        )
                        if len(deform_keypoint_files) > 0:
                            newest_id = int(
                                deform_keypoint_files[-1].split(".")[0].split("_")[-1]
                            )
                        else:
                            newest_id = -1
                        save_obj(
                            os.path.join(
                                self.args.run_path,
                                f"deform_keypoints_{newest_id+1}.pickle",
                            ),
                            [self.deform_keypoints, self.animation_time],
                        )

                    dpg.add_button(
                        label="sv_kpt",
                        tag="_button_save_kpt",
        ...
        def callback_keypoint_drag(sender, app_data):
            ...
            if dpg.is_key_down(dpg.mvKey_R):
                ...
                delta = (
                    0.00010 * self.cam.rot.as_matrix()[:3, :3] @ np.array([dx, -dy, 0])
                )
            ...
            if self.deform_mode.startswith("arap"):
                with torch.no_grad():
                    if (
                        ...
                        or self.animation_trans_bias is None
                    ):
                        init_verts = None
                    else:
                        init_verts = (
                            self.animation_trans_bias + self.animate_tool.init_pcl
                        )
        ...
        def callback_keypoint_add(sender, app_data):
            ...
            if (
                dpg.is_key_down(dpg.mvKey_S)
                or dpg.is_key_down(dpg.mvKey_D)
                or dpg.is_key_down(dpg.mvKey_F)
                or dpg.is_key_down(dpg.mvKey_A)
                or dpg.is_key_down(dpg.mvKey_Q)
            ):
                if not self.is_animation:
                    print("Please switch to animation mode!")
                    return
                ...
                with torch.no_grad():
                    ...
                    d_values = self.deform.step(
                        ...
                        node_rot_bias=self.animation_rot_bias,
                        node_scaling_bias=self.animation_scaling_bias,
                    )
                    ...
                    d = out["depth"][0][ph, pw]
                    z = cur_cam.zfar / (
                        cur_cam.zfar - cur_cam.znear
                    ) * d - cur_cam.zfar * cur_cam.znear / (
                        cur_cam.zfar - cur_cam.znear
                    )
                    uvz = (
                        torch.tensor(
                    ...
                    keypoint_idxs = torch.tensor(
                        [(p3d - nodes).norm(dim=-1).argmin()]
                    ).cuda()

                if dpg.is_key_down(dpg.mvKey_A):
                    if True:
                        keypoint_idxs = self.animate_tool.add_n_ring_nbs(
                            keypoint_idxs, n=self.n_rings_N
                        )
                    keypoint_3ds = nodes[keypoint_idxs]
                    self.deform_keypoints.add_kpts(keypoint_3ds, keypoint_idxs)
                    print(
                        f"Add kpt: {self.deform_keypoints.selective_keypoints_idx_list}"
                    )
                    ...
                    keypoint_3ds = nodes[keypoint_idxs]
                    self.deform_keypoints.add_kpts(
                        keypoint_3ds, keypoint_idxs, expand=True
                    )
                    print(
                        f"Expand kpt: {self.deform_keypoints.selective_keypoints_idx_list}"
                    )
                    ...
                    print(
                        f"Add all the control points as kpt: {self.deform_keypoints.selective_keypoints_idx_list}"
                    )
        ...
        with dpg.handler_registry():
            ...
            dpg.add_mouse_drag_handler(
                button=dpg.mvMouseButton_Right, callback=callback_keypoint_drag
            )
            dpg.add_mouse_click_handler(
                button=dpg.mvMouseButton_Left, callback=callback_keypoint_add
            )
    ...
    def draw_gs_trajectory(self, time_gap=0.3, samp_num=512, gs_num=512, thickness=1):
        fid = (
            torch.tensor(self.animation_time).cuda().float()
            if self.is_animation
            else torch.remainder(
                torch.tensor((time.time() - self.t0) * self.fps_of_fid).float().cuda()
                / len(self.scene.getTrainCameras())
                * self.video_speed,
                1.0,
            )
        )
        ...
        if os.path.exists(
            os.path.join(self.args.run_path, "trajectory_camera.pickle")
        ):
            print(
                "Use fixed camera for screenshot: ",
                os.path.join(self.args.run_path, "trajectory_camera.pickle"),
            )
            cur_cam = load_obj(
                os.path.join(self.args.run_path, "trajectory_camera.pickle")
            ...
            save_obj(
                os.path.join(self.args.run_path, "trajectory_camera.pickle"), cur_cam
            )
        fid = cur_cam.fid
        ...
        t = fid
        time_input = (
            t.unsqueeze(0).expand(self.gaussians.get_xyz.shape[0], -1)
            if self.deform.name == "mlp"
            else self.deform.deform.expand_time(t)
        )
        d_values = self.deform.step(
            self.gaussians.get_xyz.detach(),
            time_input,
        ...
        if not os.path.exists(
            os.path.join(self.args.run_path, "trajectory_keypoints.pickle")
        ):
            ...
            save_obj(
                os.path.join(self.args.run_path, "trajectory_keypoints.pickle"),
                cur_pts[pts_idx].detach().cpu().numpy(),
            )
        else:
            print(
                "Load keypoints from ",
            ...
            _, idxs, _ = pytorch3d.ops.knn_points(
                kpts[None], cur_pts[None], None, None, K=1
            )
            pts_idx = idxs[0, :, 0]
        ...
        for i in range(samp_num):
            t = fid + delta_ts[i]
            time_input = (
                t.unsqueeze(0).expand(gs_num, -1)
                if self.deform.name == "mlp"
                else self.deform.deform.expand_time(t)
            )
            d_values = self.deform.step(
                self.gaussians.get_xyz[pts_idx].detach(),
                time_input,
                feature=self.gaussians.feature[pts_idx],
                is_training=False,
                motion_mask=self.gaussians.motion_mask[pts_idx],
            )
            ...
            cur_pts2d = (
                (cur_pts2d + 1)
                / 2
                * torch.tensor([cur_cam.image_height, cur_cam.image_width]).cuda()
            )
        ...
        colors = np.array(
            [
                np.array(color_map(i / max(1, float(gs_num - 1)))[:3]) * 255
        ...
        for i in range(gs_num):
            alpha_img = cv2.polylines(
                img=alpha_img,
                pts=[traj_pts[i].astype(np.int32)],
                isClosed=False,
                color=[1, 1, 1],
                thickness=thickness,
            )
            color = colors[i] / 255
            traj_img = cv2.polylines(
                img=traj_img,
                pts=[traj_pts[i].astype(np.int32)],
                isClosed=False,
        ...
        Image.fromarray(traj_img.astype("uint8")).save(
            os.path.join(self.args.run_path, "trajectory.png")
        )
        ...
        img_begin = (
            img_begin.permute(1, 2, 0).clamp(0, 1).detach().cpu().numpy() * 255
        ).astype("uint8")
        img_end = (
            img_end.permute(1, 2, 0).clamp(0, 1).detach().cpu().numpy() * 255
        ).astype("uint8")
        Image.fromarray(img_begin).save(
            os.path.join(self.args.run_path, "traj_start.png")
        )
        Image.fromarray(img_end).save(
            os.path.join(self.args.run_path, "traj_end.png")
        )
    ...
    def render(self):
        ...
        while dpg.is_dearpygui_running():
            ...
            if self.training:
                if (
                    self.deform.name == "node"
                    and self.iteration_node_rendering
                    < self.opt.iterations_node_rendering
                ):
            ...
            if self.should_render_customized_trajectory:
                self.render_customized_trajectory(
                    use_spiral=self.should_render_customized_trajectory_spiral
                )
            self.test_step()
    ...
    def train_step(self):
        ...
        while network_gui.conn != None:
            try:
                ...
                if do_training and (
                    (self.iteration < int(self.opt.iterations)) or not keep_alive
                ):
                    break
        ...
        if not self.viewpoint_stack:
            if self.opt.progressive_train and self.iteration < int(
                self.opt.progressive_stage_steps / self.opt.progressive_stage_ratio
            ):
                cameras_to_train_idx = int(
                    min(
                        ((self.iteration) / self.opt.progressive_stage_steps + 1)
                        * self.opt.progressive_stage_ratio,
                        1.0,
                    )
                    * len(self.scene.getTrainCameras())
                )
                ...
                interval_len = int(
                    len(self.scene.getTrainCameras()) * self.opt.progressive_stage_ratio
                )
                min_idx = max(0, cameras_to_train_idx - interval_len)
                sorted_train_cams = sorted(
                    self.scene.getTrainCameras().copy(), key=lambda x: x.fid
                )
                ...
                if len(out_domain_idx) >= interval_len:
                    out_domain_idx = np.random.choice(
                        out_domain_idx, [interval_len], replace=False
                    )
                    out_domain_stack = [
                        sorted_train_cams[idx] for idx in out_domain_idx
                    ]
        ...
        if self.deform.name == "mlp" or self.deform.name == "static":
            if self.iteration < self.opt.warm_up:
                ...
                ast_noise = (
                    ...
                    * time_interval
                    * self.smooth_term(self.iteration)
                )
                d_values = self.deform.step(
                    self.gaussians.get_xyz.detach(),
            ...
            if self.iteration < self.opt.warm_up:
                d_xyz, d_rotation, d_scaling, d_opacity, d_color = (
                    ...
                    d_rotation.detach(),
                    d_scaling.detach(),
                    d_opacity.detach() if d_opacity is not None else None,
                    d_color.detach() if d_color is not None else None,
                )
        ...
        if random_bg_color:
            ...
            gt_image = (
                ...
                + (1 - gt_alpha_mask) * render_pkg_re["bg_color"][:, None, None]
            )
        elif (
        ...
        loss_img = (1.0 - self.opt.lambda_dssim) * Ll1 + self.opt.lambda_dssim * (
        ...
        lambda_optical = landmark_interpolate(
            self.opt.lambda_optical_landmarks,
            self.opt.lambda_optical_steps,
            self.iteration,
        )
        if (
            flow_id2_candidates != []
            and lambda_optical > 0
            and self.iteration >= self.opt.warm_up
        ):
            # Pick flow file and read it
            flow_id2_dir = np.random.choice(flow_id2_candidates)
            flow = np.load(flow_id2_dir)
            mask_id2_dir = flow_id2_dir.replace(
                "raft_neighbouring", "raft_masks"
            ).replace(".npy", ".png")
            masks = imageio.imread(mask_id2_dir) / 255.0
            ...
            if flow.shape[0] != image.shape[1] or flow.shape[1] != image.shape[2]:
                flow = torch.nn.functional.interpolate(
                    flow.permute([2, 0, 1])[None], (image.shape[1], image.shape[2])
                )[0].permute(1, 2, 0)
                masks = torch.nn.functional.interpolate(
                    masks.permute([2, 0, 1])[None], (image.shape[1], image.shape[2])
                )[0].permute(1, 2, 0)
            ...
            if not hasattr(self, "img2cam"):
                self.img2cam = {
                    cam.image_name: idx
                    for idx, cam in enumerate(self.scene.getTrainCameras().copy())
                }
            if cam2_id in self.img2cam:  # Only considering the case with existing files
                ...
                coor1to2_flow = (
                    flow
                    / torch.tensor(flow.shape[:2][::-1], dtype=torch.float32).cuda()
                    * 2
                )
                ...
                pair_weight = torch.clamp(
                    torch.cos((fid1 - fid2).abs() * np.pi / 2), 0.2, 1
                )
                ...
                d_xyz2 = self.deform.step(
                    self.gaussians.get_xyz.detach(),
                    time_input2 + ast_noise,
                    iteration=self.iteration,
                    feature=self.gaussians.feature,
                    motion_mask=self.gaussians.motion_mask,
                    camera_center=viewpoint_cam2.camera_center,
                )["d_xyz"]
                # Render the flow image
                render_pkg2 = render_flow(
                    pc=self.gaussians,
                    viewpoint_camera1=viewpoint_cam,
                    viewpoint_camera2=viewpoint_cam2,
                    d_xyz1=d_xyz,
                    d_xyz2=d_xyz2,
                    d_rotation1=d_rotation,
                    d_scaling1=d_scaling,
                    scale_const=None,
                )
                coor1to2_motion = render_pkg2["render"][:2].permute(1, 2, 0)
                mask_motion = (
                    render_pkg2["alpha"][0] > 0.9
                ).detach()  # Only optimizing the space with solid points to avoid dilation
        ...
        lambda_motion_mask = landmark_interpolate(
            self.opt.lambda_motion_mask_landmarks,
            self.opt.lambda_motion_mask_steps,
            self.iteration,
        )
        if (
            not self.opt.no_motion_mask_loss
            and self.deform.name == "node"
            and self.opt.gt_alpha_mask_as_dynamic_mask
            and viewpoint_cam.gt_alpha_mask is not None
            and lambda_motion_mask > 0
        ):
            gt_alpha_mask = viewpoint_cam.gt_alpha_mask.cuda()
            render_pkg_motion = render(
                viewpoint_cam,
                self.gaussians,
                self.pipe,
                self.background,
        ...
        with torch.no_grad():
            ...
            if (
                self.iteration in self.saving_iterations
                or self.iteration == self.best_iteration
                or self.iteration == self.opt.warm_up - 1
            ):
                print("\n[ITER {}] Saving Gaussians".format(self.iteration))
                self.scene.save(self.iteration)
                self.deform.save_weights(self.args.run_path, self.iteration)
            ...
            if self.iteration < self.opt.densify_until_iter:
                ...
                if (
                    self.iteration > self.opt.node_densify_from_iter
                    and self.iteration % self.opt.node_densification_interval == 0
                    and self.iteration < self.opt.node_densify_until_iter
                    and self.iteration > self.opt.warm_up
                    or self.iteration == self.opt.node_force_densify_prune_step
                ):
                    # Nodes densify
                    self.deform.densify(
                        max_grad=self.opt.densify_grad_threshold,
                        x=self.gaussians.get_xyz,
                        x_grad=self.gaussians.xyz_gradient_accum / self.gaussians.denom,
                        feature=self.gaussians.feature,
                        force_dp=(
                            self.iteration == self.opt.node_force_densify_prune_step
                        ),
                    )
    ...
    def train_node_rendering_step(self):
        ...
        if not self.viewpoint_stack:
            if (
                self.opt.progressive_train_node
                and self.iteration_node_rendering
                < int(
                    self.opt.progressive_stage_steps / self.opt.progressive_stage_ratio
                )
                + self.opt.node_warm_up
            ):
                if self.iteration_node_rendering < self.opt.node_warm_up:
                    ...
                    cameras_to_train_idx = int(
                        min(
                            (
                                (self.iteration_node_rendering - self.opt.node_warm_up)
                                / self.opt.progressive_stage_steps
                                + 1
                            )
                            * self.opt.progressive_stage_ratio,
                            1.0,
                    ...
                    viewpoint_stack = sorted_train_cams[min_idx:cameras_to_train_idx]
                    out_domain_idx = np.concatenate(
                        [
                            np.arange(min_idx),
                            np.arange(
                                cameras_to_train_idx,
                                min(
                                    len(self.scene.getTrainCameras()),
                                    cameras_to_train_idx + interval_len,
                                ),
                            ),
                        ]
                    )
                    if len(out_domain_idx) >= interval_len:
                        ...
                        out_domain_idx = np.random.choice(
                            out_domain_idx, [out_domain_len], replace=False
                        )
                        out_domain_stack = [
                            sorted_train_cams[idx] for idx in out_domain_idx
                        ]
        ...
        time_input = fid.unsqueeze(0).expand(
            self.deform.deform.as_gaussians.get_xyz.shape[0], -1
        )
        N = time_input.shape[0]
        ...
        ast_noise = (
            ...
            * time_interval
            * self.smooth_term(self.iteration_node_rendering)
        )
        d_values = self.deform.deform.query_network(
            x=self.deform.deform.as_gaussians.get_xyz.detach(), t=time_input + ast_noise
        )
        d_xyz, d_opacity, d_color = (
            d_values["d_xyz"] * self.deform.deform.as_gaussians.motion_mask,
            (
                d_values["d_opacity"] * self.deform.deform.as_gaussians.motion_mask
                if d_values["d_opacity"] is not None
                else None
            ),
            (
                d_values["d_color"] * self.deform.deform.as_gaussians.motion_mask
                if d_values["d_color"] is not None
                else None
            ),
        )
        d_rot, d_scale = 0.0, 0.0
        if self.iteration_node_rendering < self.opt.node_warm_up:
            d_xyz = d_xyz.detach()
        d_color = d_color.detach() if d_color is not None else None
        ...
        # Render
        random_bg_color = (
            self.opt.gt_alpha_mask_as_scene_mask
            or (
                self.opt.gt_alpha_mask_as_dynamic_mask
                and not self.deform.deform.as_gaussians.with_motion_mask
            )
        ) and viewpoint_cam.gt_alpha_mask is not None
        render_pkg_re = render(
            viewpoint_cam,
        ...
        if self.iteration_node_rendering > self.opt.node_warm_up:
            if not self.deform.deform.use_hash:
                elastic_loss = 1e-3 * self.deform.deform.elastic_loss(
                    t=fid, delta_t=time_interval
                )
                loss_acc = 1e-5 * self.deform.deform.acc_loss(
                    t=fid, delta_t=3 * time_interval
                )
        ...
        if (
            self.opt.gt_alpha_mask_as_dynamic_mask
            and self.deform.deform.as_gaussians.with_motion_mask
            and viewpoint_cam.gt_alpha_mask is not None
            and self.iteration_node_rendering > self.opt.node_warm_up
        ):
            ...
            render_pkg_motion = render(
                ...
                detach_rot=True,
                detach_scale=True,
                detach_opacity=self.deform.deform.as_gaussians.with_motion_mask,
                d_opacity=d_opacity,
                d_color=d_color,
                d_rot_as_res=self.deform.d_rot_as_res,
            )
        ...
        with torch.no_grad():
            if self.iteration_node_rendering < self.opt.iterations_node_sampling:
                # Densification
                self.deform.deform.as_gaussians.add_densification_stats(
                    viewspace_point_tensor, visibility_filter
                )
                if (
                    self.iteration_node_rendering % self.opt.densification_interval == 0
                    or self.iteration_node_rendering == self.opt.node_warm_up - 1
                ):
                    size_threshold = (
                        20
                        if self.iteration_node_rendering
                        > self.opt.opacity_reset_interval
                        else None
                    )
                    if self.dataset.is_blender:
                        ...
                        if (
                            self.deform.deform.as_gaussians.get_xyz.shape[0]
                            > self.deform.deform.node_num
                            * self.opt.node_max_num_ratio_during_init
                        ):
                            grad_max = torch.inf
                        else:
                            grad_max = self.opt.densify_grad_threshold
                    self.deform.deform.as_gaussians.densify_and_prune(
                        grad_max, 0.005, self.scene.cameras_extent, size_threshold
                    )
                if (
                    self.iteration_node_rendering % self.opt.opacity_reset_interval == 0
                    or (
                        self.dataset.white_background
                        and self.iteration_node_rendering == self.opt.densify_from_iter
                    )
                ):
                ...
                if strategy == "direct":
                    ...
                    self.deform.deform.init(
                        opt=self.opt,
                        init_pcl=original_gaussians.get_xyz,
                        keep_all=True,
                        force_init=True,
                        reset_bbox=False,
                        feature=self.gaussians.feature,
                    ...
                    gaussians._features_dc = torch.nn.Parameter(
                        original_gaussians._features_dc
                    )
                    gaussians._features_rest = torch.nn.Parameter(
                        original_gaussians._features_rest
                    )
                    ...
                    gaussians._rotation = torch.nn.Parameter(
                        original_gaussians._rotation
                    )
                    if gaussians.fea_dim > 0:
                        gaussians.feature = torch.nn.Parameter(
                            original_gaussians.feature
                        )
                    ...
                    for i in range(time_num):
                        ...
                        trans_samp.append(
                            self.deform.deform.query_network(x=x, t=time_input)["d_xyz"]
                            * original_gaussians.motion_mask
                        )
                    trans_samp = torch.stack(trans_samp, dim=1)
                    hyper_pcl = (
                        trans_samp + original_gaussians.get_xyz[:, None]
                    ).reshape([original_gaussians.get_xyz.shape[0], -1])
                    dynamic_mask = (
                        self.deform.deform.as_gaussians.motion_mask[..., 0] > 0.5
                    )
                    if not self.opt.deform_downsamp_with_dynamic_mask:
                        dynamic_mask = torch.ones_like(dynamic_mask)
                    idx = self.deform.deform.init(
                        init_pcl=original_gaussians.get_xyz[dynamic_mask],
                        hyper_pcl=hyper_pcl[dynamic_mask],
                        force_init=True,
                        opt=self.opt,
                        reset_bbox=False,
                        feature=self.gaussians.feature,
                    ...
                    gaussians: GaussianModel = self.deform.deform.as_gaussians
                    gaussians._features_dc = torch.nn.Parameter(
                        original_gaussians._features_dc[dynamic_mask][idx]
                    )
                    gaussians._features_rest = torch.nn.Parameter(
                        original_gaussians._features_rest[dynamic_mask][idx]
                    )
                    gaussians._scaling = torch.nn.Parameter(
                        original_gaussians._scaling[dynamic_mask][idx]
                    )
                    gaussians._opacity = torch.nn.Parameter(
                        original_gaussians._opacity[dynamic_mask][idx]
                    )
                    gaussians._rotation = torch.nn.Parameter(
                        original_gaussians._rotation[dynamic_mask][idx]
                    )
                    if gaussians.fea_dim > 0:
                        gaussians.feature = torch.nn.Parameter(
                            original_gaussians.feature[dynamic_mask][idx]
                        )
            ...
            if (
                self.iteration_node_rendering == self.opt.iterations_node_rendering - 1
                and self.iteration_node_rendering > self.opt.iterations_node_sampling
            ):
                # Just finish node training and has down sampled control nodes
                self.deform.deform.nodes.data[..., :3] = (
                    self.deform.deform.as_gaussians._xyz
                )

            if (
                not self.iteration_node_rendering == self.opt.iterations_node_sampling
                and not self.iteration_node_rendering
                == self.opt.iterations_node_rendering - 1
            ):
                # Optimizer step
                self.deform.deform.as_gaussians.optimizer.step()
                self.deform.deform.as_gaussians.update_learning_rate(
                    self.iteration_node_rendering
                )
                self.deform.deform.as_gaussians.optimizer.zero_grad(set_to_none=True)
        ...
        self.deform.update(
            max(0, self.iteration_node_rendering - self.opt.node_warm_up)
        )
    ...
    def test_step(self, specified_cam=None):
        ...
        if self.should_save_screenshot and os.path.exists(
            os.path.join(self.args.run_path, "screenshot_camera.pickle")
        ):
            print(
                "Use fixed camera for screenshot: ",
                os.path.join(self.args.run_path, "screenshot_camera.pickle"),
        ...
        if self.deform.name == "node":
            if "Node" in self.visualization_mode:
                ...
                time_input = fid.unsqueeze(0).expand(gaussians.get_xyz.shape[0], -1)
                d_values = self.deform.deform.query_network(
                    x=gaussians.get_xyz.detach(), t=time_input
                )
                ...
                d_xyz, d_opacity, d_color = (
                    d_values["d_xyz"] * gaussians.motion_mask,
                    (
                        d_values["d_opacity"] * gaussians.motion_mask
                        if d_values["d_opacity"] is not None
                        else None
                    ),
                    (
                        d_values["d_color"] * gaussians.motion_mask
                        if d_values["d_color"] is not None
                        else None
                    ),
                )
                ...
                d_rotation_bias = (
                    d_values["d_rotation_bias"]
                    if "d_rotation_bias" in d_values.keys()
                    else None
                )
        else:
            vis_scale_const = None
        ...
        if self.vis_traj_realtime:
            if "Node" in self.visualization_mode:
                if self.last_traj_overlay_type != "node":
                    self.traj_coor = None
                self.update_trajectory_overlay(
                    gs_xyz=gaussians.get_xyz + d_xyz, camera=cur_cam, gs_num=512
                )
                ...
                if self.last_traj_overlay_type != "gs":
                    self.traj_coor = None
                self.update_trajectory_overlay(
                    gs_xyz=gaussians.get_xyz + d_xyz, camera=cur_cam
                )
        ...
        if self.visualization_mode == "Dynamic" or self.visualization_mode == "Static":
            ...
            if self.visualization_mode == "Dynamic":
                d_opacity[self.gaussians.motion_mask < 0.9] = -1e3
            else:
                d_opacity[self.gaussians.motion_mask > 0.1] = -1e3

        render_motion = "Motion" in self.visualization_mode
        if render_motion:
            vis_scale_const = self.vis_scale_const
        if (
            type(d_rotation) is not float
            and gaussians._rotation.shape[0] != d_rotation.shape[0]
        ):
        ...
        if self.should_save_screenshot:
            alpha = out["alpha"]
            sv_image = (
                torch.cat([buffer_image, alpha], dim=0)
                .clamp(0, 1)
                .permute(1, 2, 0)
                .detach()
                .cpu()
            ...
            def save_image(image, image_dir):
                ...
                print(
                    ">>> Saving image to %s" % os.path.join(image_dir, "%05d.png" % idx)
                )
                Image.fromarray((image * 255).astype("uint8")).save(
                    os.path.join(image_dir, "%05d.png" % idx)
                )
        ...
        if self.vis_traj_realtime:
            buffer_image = (
                buffer_image * (1 - self.traj_overlay[..., 3:])
                + self.traj_overlay[..., :3] * self.traj_overlay[..., 3:]
            )

        if self.gui:
            dpg.set_value(
                "_log_infer_time", f"{t:.4f}ms ({int(1000/t)} FPS FID: {fid.item()})"
            )
            dpg.set_value(
    ...
    def update_control_point_overlay(self):
        ...
        if self.need_update_overlay and len(self.deform_keypoints.get_kpt()) > 0:
            try:
                ...
                source_points_clip = np.matmul(
                    np.pad(source_points, ((0, 0), (0, 1)), constant_values=1.0), mvp.T
                )  # [N, 4]
                target_points_clip = np.matmul(
                    np.pad(target_points, ((0, 0), (0, 1)), constant_values=1.0), mvp.T
                )  # [N, 4]
                source_points_clip[:, :3] /= source_points_clip[
                    :, 3:
                ]  # perspective division
                target_points_clip[:, :3] /= target_points_clip[
                    :, 3:
                ]  # perspective division

                source_points_2d = (
                    (((source_points_clip[:, :2] + 1) / 2) * np.array([self.H, self.W]))
                    .round()
                    .astype(np.int32)
                )
                target_points_2d = (
                    (((target_points_clip[:, :2] + 1) / 2) * np.array([self.H, self.W]))
                    .round()
                    .astype(np.int32)
                )
                ...
                keypoint_idxs_to_drag = (
                    self.deform_keypoints.selective_keypoints_idx_list
                )
                for i in range(len(source_points_clip)):
                    point_idx = points_indices[i]
                    # draw source point
                    if (
                        source_points_2d[i, 0] >= radius
                        and source_points_2d[i, 0] < self.W - radius
                        and source_points_2d[i, 1] >= radius
                        and source_points_2d[i, 1] < self.H - radius
                    ):
                        buffer_overlay[
                            source_points_2d[i, 1]
                            - radius : source_points_2d[i, 1]
                            + radius,
                            source_points_2d[i, 0]
                            - radius : source_points_2d[i, 0]
                            + radius,
                        ] += (
                            np.array([1, 0, 0])
                            if not point_idx in keypoint_idxs_to_drag
                            else np.array([1, 0.87, 0])
                        )
                        # draw target point
                        if (
                            target_points_2d[i, 0] >= radius
                            and target_points_2d[i, 0] < self.W - radius
                            and target_points_2d[i, 1] >= radius
                            and target_points_2d[i, 1] < self.H - radius
                        ):
                            buffer_overlay[
                                target_points_2d[i, 1]
                                - radius : target_points_2d[i, 1]
                                + radius,
                                target_points_2d[i, 0]
                                - radius : target_points_2d[i, 0]
                                + radius,
                            ] += (
                                np.array([0, 0, 1])
                                if not point_idx in keypoint_idxs_to_drag
                                else np.array([0.5, 0.5, 1])
                            )
                        # draw line
                        rr, cc, val = line_aa(
                            source_points_2d[i, 1],
                        ...
                        in_canvas_mask = (
                            (rr >= 0) & (rr < self.H) & (cc >= 0) & (cc < self.W)
                        )
                        buffer_overlay[rr[in_canvas_mask], cc[in_canvas_mask]] += (
                            val[in_canvas_mask, None] * np.array([0, 1, 0])
                            if not point_idx in keypoint_idxs_to_drag
    ...
    def update_trajectory_overlay(
        self, gs_xyz, camera, samp_num=32, gs_num=512, thickness=1
    ):
        if not hasattr(self, "traj_coor") or self.traj_coor is None:
            ...
            opacity_mask = (
                self.gaussians.get_opacity[..., 0] > 0.1
                if self.gaussians.get_xyz.shape[0] == gs_xyz.shape[0]
                else torch.ones_like(gs_xyz[:, 0], dtype=torch.bool)
            )
            masked_idx = torch.arange(
                0, opacity_mask.shape[0], device=opacity_mask.device
            )[opacity_mask]
            self.traj_idx = masked_idx[
                farthest_point_sample(gs_xyz[None, opacity_mask], gs_num)[0]
            ]
            from matplotlib import cm
        ...
        traj_uv = (
            (traj_uv + 1)
            / 2
            * torch.tensor([camera.image_height, camera.image_width]).cuda()
        )
        ...
        colors = np.array(
            [
                np.array(self.traj_color_map(i / max(1, float(gs_num - 1)))[:3]) * 255
                for i in range(gs_num)
            ],
            dtype=np.int32,
        )
        alpha_img = np.zeros(
            [camera.image_height, camera.image_width, 3], dtype=np.float32
        )
        traj_img = np.zeros(
            [camera.image_height, camera.image_width, 3], dtype=np.float32
        )
    ...
    def render_customized_trajectory(
        self, use_spiral=False, traj_dir=None, fps=30, motion_repeat=1
    ):
        from utils.pickle_utils import load_obj
        ...
        if traj_dir is None:
            traj_dir = os.path.join(self.args.run_path, "trajectory")
        # Read deformation files for animation presentation
        deform_keypoint_files = [None] + sorted(
            [
                file
                for file in os.listdir(os.path.join(self.args.run_path))
                if file.startswith("deform_keypoints") and file.endswith(".pickle")
            ]
        )
        rendering_animation = len(deform_keypoint_files) > 0
        if rendering_animation:
            deform_keypoints, self.animation_time = load_obj(
                os.path.join(self.args.run_path, deform_keypoint_files[1])
            )
            self.animation_initialize()
        # Read camera trajectory files
        if os.path.exists(traj_dir):
            cameras = sorted(
                [cam for cam in os.listdir(traj_dir) if cam.endswith(".pickle")]
            )
            ...
            poses = render_path_spiral(
                c2ws=c2ws, focal=self.cam.fovx * 200, rots=3, N=30 * 12
            )
        ...
        def min_line_dist_center(rays_o, rays_d):
            try:
                ...
                pt_mindist = np.squeeze(
                    -np.linalg.inv((A_i @ A_i).mean(0)) @ (b_i).mean(0)
                )
            except:
                pt_mindist = None
            return pt_mindist
        ...
        if rendering_animation:
            ...
            for deform_keypoint_file in deform_keypoint_files:
                if deform_keypoint_file is None:
                    ...
                    deform_keypoints = load_obj(
                        os.path.join(self.args.run_path, deform_keypoint_file)
                    )[0]
                    animated_pcl, _, _ = self.animate_tool.deform_arap(
                        handle_idx=deform_keypoints.get_kpt_idx(),
                        handle_pos=deform_keypoints.get_deformed_kpt_np(),
                        return_R=True,
                    )
        ...
        # Save path
        sv_dir = os.path.join(self.args.run_path, "render_trajectory")
        os.makedirs(sv_dir, exist_ok=True)
        import cv2

        video = cv2.VideoWriter(
            sv_dir + f"/{self.mode}.mp4",
        ...
        for i in range(len(cameras) - 1):
            if use_spiral:
                ...
                avg_center = min_line_dist_center(
                    c2ws[i : i + 2, :3, 3], c2ws[i : i + 2, :3, 2]
                )
                if avg_center is not None:
                    vec1_norm1, vec2_norm = np.linalg.norm(
                        c2ws[i, :3, 3] - avg_center
                    ), np.linalg.norm(c2ws[i + 1, :3, 3] - avg_center)
                    slerp_t = geometric_slerp(
                        start=(c2ws[i, :3, 3] - avg_center) / vec1_norm1,
                        end=(c2ws[i + 1, :3, 3] - avg_center) / vec2_norm,
                ...
                for j in range(frame_num):
                    ...
                    if avg_center is not None:
                        trans = (
                            slerp_t[j] * (vec1_norm1 + (vec2_norm - vec1_norm1) * rate)
                            + avg_center
                        )
                    else:
                        trans = (
                            c2ws[i, :3, 3]
                            + (c2ws[i + 1, :3, 3] - c2ws[i, :3, 3]) * rate
                        )
                    c2w = np.eye(4)
                    c2w[:3, :3] = rot
                    c2w[:3, 3] = trans
                    ...
                    vis_cams.append(c2w)
                    fid = (
                        cam0.fid + (cam1.fid - cam0.fid) * rate
                        if not rendering_animation
                        else torch.tensor(self.animation_time).cuda().float()
                    )
                    cam = MiniCam(
                        c2w=c2w,
                        width=cam0.image_width,
                        height=cam0.image_height,
                        fovy=cam0.FoVy,
                        fovx=cam0.FoVx,
                        znear=cam0.znear,
                        zfar=cam0.zfar,
...
def prepare_output_and_logger(args):
    if not args.run_path:
        ...
        args.run_path = os.path.join("./output/", unique_str[0:10])

    # Set up output folder
    print("Output folder: {}".format(args.run_path))
    os.makedirs(args.run_path, exist_ok=True)
    with open(os.path.join(args.run_path, "cfg_args"), "w") as cfg_log_f:
        cfg_log_f.write(str(Namespace(**vars(args))))

    # Create Tensorboard writer
    tb_writer = None
    if TENSORBOARD_FOUND:
        tb_writer = SummaryWriter(args.run_path)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--elevation", type=float, default=0, help="default GUI camera elevation"
    )
    parser.add_argument(
    ...
    # if not args.run_path.endswith(args.deform_type):
    #     args.run_path = os.path.join(os.path.dirname(os.path.normpath(args.run_path)), os.path.basename(os.path.normpath(args.run_path)) + f'_{args.deform_type}')

    print("Optimizing " + args.run_path)
