...
class DeformModel:
    def __init__(
        self, deform_type="node", is_blender=False, d_rot_as_res=True, **kwargs
    ):
        self.deform = model_dict[deform_type](
            is_blender=is_blender, d_rot_as_res=d_rot_as_res, **kwargs
        ).cuda()
    ...
    def save_weights(self, run_path, iteration):
        out_weights_path = os.path.join(
            run_path, "deform/iteration_{}".format(iteration)
        )
    ...
    def load_weights(self, run_path, iteration=-1):
        if iteration == -1:
            loaded_iter = searchForMaxIteration(os.path.join(run_path, "deform"))
        else:
            loaded_iter = iteration
        weights_path = os.path.join(
            run_path, "deform/iteration_{}/deform.pth".format(loaded_iter)
        )
    ...
    def update_learning_rate(self, iteration):
        for param_group in self.optimizer.param_groups:
            if (
                param_group["name"] == "deform"
                or param_group["name"] == "mlp"
                or "node" in param_group["name"]
            ):
