...
class Scene:
    ...
    def __init__(
        ...
        if load_iteration:
            if load_iteration == -1:
                self.loaded_iter = searchForMaxIteration(
                    os.path.join(self.run_path, "point_cloud")
                )
            else:
        ...
        if os.path.exists(os.path.join(args.dataset_path, "sparse")) or os.path.exists(
            os.path.join(args.dataset_path, "colmap_sparse")
        ):
            scene_info = sceneLoadTypeCallbacks["Colmap"](
                args.dataset_path, args.images, args.eval
            )
        elif os.path.exists(os.path.join(args.dataset_path, "transforms_train.json")):
            print("Found transforms_train.json file, assuming Blender data set!")
            scene_info = sceneLoadTypeCallbacks["Blender"](
                args.dataset_path, args.white_background, args.eval
            )
            ...
            scene_info = sceneLoadTypeCallbacks["DTU"](
                args.dataset_path, "cameras_sphere.npz", "cameras_sphere.npz"
            )
            ...
            scene_info = sceneLoadTypeCallbacks["plenopticVideo"](
                args.dataset_path, args.eval, 24
            )
        ...
        if not self.loaded_iter:
            with open(scene_info.ply_path, "rb") as src_file, open(
                os.path.join(self.run_path, "input.ply"), "wb"
            ) as dest_file:
            ...
            with open(os.path.join(self.run_path, "cameras.json"), "w") as file:
        ...
        for cam in scene_info.train_cameras:
            flow_dirs_list.append(
                [
                    os.path.join(self.flow_dir, flow_dir)
                    for flow_dir in flow_list
                    if flow_dir.startswith(cam.image_name + ".")
                ]
            )
        ...
        for resolution_scale in resolution_scales:
            ...
            self.train_cameras[resolution_scale] = cameraList_from_camInfos(
                scene_info.train_cameras, resolution_scale, args, flow_dirs_list
            )
    ...
    def save(self, iteration):
        point_cloud_path = os.path.join(
            self.run_path, "point_cloud/iteration_{}".format(iteration)
        )
