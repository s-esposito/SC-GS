...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, msk_folder=None):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if intr.model == "SIMPLE_PINHOLE":
            ...
            intr.model == "PINHOLE"
            or intr.model == "OPENCV"
            or intr.model == "SIMPLE_RADIAL"
        ):
            focal_length_x = intr.params[0]
...
def readColmapSceneInfo(
    path, images, eval, llffhold=16, apply_cam_norm=False, recenter_by_pcl=False
):
    sparse_name = (
        "sparse" if os.path.exists(os.path.join(path, "sparse")) else "colmap_sparse"
    )
    try:
    ...
    if eval:
        train_cam_infos = [
            c for idx, c in enumerate(cam_infos) if idx % llffhold != 0 or True
        ]  # Test all files with flow
    ...
    nerf_normalization = getNerfppNorm(
        train_cam_infos + test_cam_infos, apply=apply_cam_norm
    )
...
def readCamerasFromTransforms(
    path, transformsfile, white_background, extension=".png", no_bg=False
):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        frames = sorted(
            frames,
            key=lambda x: int(
                os.path.basename(x["file_path"]).split(".")[0].split("_")[-1]
            ),
        )
        for idx, frame in enumerate(frames):
            ...
            if os.path.exists(
                os.path.join(
                    os.path.dirname(
                        os.path.dirname(os.path.join(path, frame["file_path"]))
                    ),
                    "rgba",
                )
            ):
            ...
            arr = norm_data[:, :, :3]
            if no_bg:
                norm_data[:, :, :3] = norm_data[:, :, 3:4] * norm_data[
                    :, :, :3
                ] + bg * (1 - norm_data[:, :, 3:4])
            ...
            image = Image.fromarray(
                np.array(arr * 255.0, dtype=np.byte),
                "RGBA" if arr.shape[-1] == 4 else "RGB",
            )
    ...
    if not os.path.exists(ply_path):
        if os.path.exists(os.path.join(path, "rgbd")):
            ...
            gt_depth = torch.from_numpy(
                load_depth(os.path.join(path, "rgbd", "0.depth"))
            )
            gt_depth = torch.nn.functional.interpolate(gt_depth[None, None], (h, w))[
                0, 0
            ]
            far, near = viewpoint_camera.zfar, viewpoint_camera.znear
            u, v = torch.meshgrid(
                torch.linspace(0.5, w - 0.5, w, device=gt_depth.device) / w * 2 - 1,
                torch.linspace(0.5, h - 0.5, h, device=gt_depth.device) / h * 2 - 1,
                indexing="xy",
            )
            ...
            num_pts = shs.shape[0]
            pcd = BasicPointCloud(
                points=pcl, colors=SH2RGB(shs), normals=np.zeros((num_pts, 3))
            )
            xyz = pcl
        else:
...
def readNerfiesCameras(path, inter_valid=True):
    ...
    if name.startswith("vrig"):
        ...
        train_img = list(
            np.array(dataset_json["ids"])[
                [i for i in range(len(dataset_json["ids"])) if i % 4 == 0]
            ]
        )
        val_img = list(
            np.array(dataset_json["ids"])[
                [i for i in range(len(dataset_json["ids"])) if i % 4 == 2]
            ]
        )
        all_img = train_img + val_img
        ratio = 0.5
    ...
    if os.path.exists(f"{path}/rgb/rgba"):
        ...
        msk_path = [
            f"{path}/resized_mask/{int(1 / ratio)}x/{i}.png.png" for i in all_img
        ]
        msk_path = (
            msk_path
            if os.path.exists(f"{path}/resized_mask/{int(1 / ratio)}x/")
            else None
        )
    ...
    if inter_valid:
        ...
        for idx in range(len(train_img)):
            ...
            if msk_path is not None:
                ...
                image = Image.fromarray(
                    np.concatenate([np.asarray(image), mask], axis=-1).astype("uint8")
                )
...
def readNerfiesColmapCameras(path):
    ...
    sparse_name = (
        "sparse"
        if os.path.exists(os.path.join(path, "colmap", "sparse"))
        else "colmap_sparse"
    )
    cameras_extrinsic_file = os.path.join(
        path, "colmap", f"{sparse_name}/0", "images.bin"
    )
    cameras_intrinsic_file = os.path.join(
        path, "colmap", f"{sparse_name}/0", "cameras.bin"
    )
    cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
    cam_intrinsics = read_intrinsics_binary(cameras_intrinsic_file)
...
def readNerfiesInfo(path, eval):
    ...
    if os.path.exists(os.path.join(path, "colmap")):
        cam_infos, train_num, scene_center, scene_center = readNerfiesColmapCameras(
            path
        )
    ...
    if os.path.exists(os.path.join(path, "colmap")):
        ...
        if recenter_by_pcl:
            ply_path = os.path.join(
                path, f"colmap/{sparse_name}/0/points3d_recentered.ply"
            )
        elif apply_cam_norm:
            ply_path = os.path.join(
                path, f"colmap/{sparse_name}/0/points3d_normalized.ply"
            )
        else:
    ...
    for t in range(num_timesteps):
        for c in range(len(md["fn"][t])):
            ...
            try:
                seg = np.array(
                    copy.deepcopy(
                        Image.open(f"{path}/seg/{image_name.replace('.jpg', '.png')}")
                    )
                ).astype(np.float32)
            except:
                seg = None
...
def readCMUSceneInfo(path, apply_cam_norm=True, recenter_by_pcl=True):
    ...
    if not os.path.exists(ply_path):
        ...
        if recenter_by_pcl:
            ...
            np.savez(adj_path, translate=-pcl_center)
        pcd = BasicPointCloud(
            points=xyz, colors=SH2RGB(shs), normals=np.zeros_like(xyz)
        )
        storePly(ply_path, xyz, SH2RGB(shs) * 255)
    else:
        translate = np.load(adj_path + ".npz")["translate"]
