...
class GaussianModel:
    ...
    def get_rotation_bias(self, rotation_bias=None):
        rotation_bias = rotation_bias if rotation_bias is not None else 0.0
    ...
    def create_from_pcd(
        ...
        self.feature = nn.Parameter(
            -1e-2
            * torch.ones([self._xyz.shape[0], self.fea_dim], dtype=torch.float32).to(
                "cuda:0"
            ),
            requires_grad=True,
        )
    ...
    def load_ply(self, path, og_number_points=-1):
        ...
        fea_names = [
            p.name for p in plydata.elements[0].properties if p.name.startswith("fea")
        ]
        ...
        if self.fea_dim > 0:
            self.feature = nn.Parameter(
                torch.tensor(feas, dtype=torch.float, device="cuda").requires_grad_(
                    True
                )
            )
    ...
    def densify_and_split(
        ...
        new_feature = (
            self.feature[selected_pts_mask].repeat(N, 1) if self.fea_dim > 0 else None
        )
...
class StandardGaussianModel(GaussianModel):
    ...
    def get_scaling(self):
        scaling = (
            self._scaling.mean()[None, None].expand_as(self._scaling)
            if self.all_the_same
            else self._scaling.mean(dim=1, keepdim=True).expand_as(self._scaling)
        )
